...
from prompts import overall_evaluation_prompt, ecg_evaluation_prompt
import traceback
        ...
        if "User: " in sentence:
            ...
            if is_val: ## test
                ...
                label.append({
                    ...
                    "history": template_history.replace("\n<|eot_id|>", "\n"), #history up to the user utterance and then assistant: 
                    "dial": template_chat, #the ground-truth response, or the dialogue including the assistant's response
                    "label": current_dial['label']}) #the ground-truth assistant's response
    return label
...
def ecg_generate(current_dial, model, rank, tokenizer):
    stop_token_id = tokenizer.encode("- Message:")[-1]
    stopping_criteria = StoppingCriteriaList([StopOnToken(stop_token_id)])
    ...
    while True:
        ...
        # Handle ECG-specific tool calls
        if "- Action: call_classification_tool" in generated:
            obs_output = input_obs.get('classification', [])
            generated += f"\n- Observation: {obs_output}\n"
        elif "- Action: call_measurement_tool" in generated:
            obs_output = input_obs.get('measurement', {})
            generated += f"\n- Observation: {obs_output}\n"
        elif "- Action: call_explanation_tool" in generated:
            obs_output = input_obs.get('explanation', [])
            generated += f"\n- Observation: {obs_output}\n"
...
if __name__ == "__main__":
    ...
    # We will call the train function directly, bypassing mp.spawn
    # This forces the entire script into a single, debuggable process.
    print("--- RUNNING IN SINGLE-PROCESS DEBUG MODE ---")

    rank = 0        # The ID for our single process
    world_size = 1  # We are only using 1 GPU/process
